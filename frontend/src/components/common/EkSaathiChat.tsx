...
export const EkSaathiChat: React.FC = () => {
    ...
    const [isMuted, setIsMuted] = useState(() => {
        return localStorage.getItem('eksaathi_muted') === 'true';
    });

    const toggleMute = () => {
        setIsMuted(prev => {
            const next = !prev;
            localStorage.setItem('eksaathi_muted', String(next));
                    ...
                    <div className="eksaathi-header">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <button
                                ...
                                style={{
                                    background: isMuted ? '#FFF5F5' : '#F0FFF4',
                                    border: `1px solid ${isMuted ? '#FED7D7' : '#C6F6D5'}`,
                                    borderRadius: '999px',
                                    padding: '4px 8px',
                                    cursor: 'pointer',
                                    display: 'flex',
