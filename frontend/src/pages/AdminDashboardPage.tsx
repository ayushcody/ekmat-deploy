...
import { Server, Plus, ToggleLeft, ToggleRight, Upload, Users, BarChart3, PieChart as PieChartIcon, TrendingUp, Activity, Wallet, Shield } from 'lucide-react';
import { BLOCK_EXPLORER_URL } from '../utils/constants';

// Helper: format epoch timestamp to readable date
...
// Helper: election status
const getElectionStatus = (e: any) => {
    const now = Math.floor(Date.now() / 1000);
    if (!e.active) return { label: 'Inactive', color: '#A0AEC0' };
    if (now < e.startTime) return { label: 'Upcoming', color: '#D69E2E' };
    if (now > e.endTime) return { label: 'Ended', color: '#E53E3E' };
    return { label: 'Live', color: '#38A169' };
};
import {
    PieChart, Pie, Cell,
...
const AdminDashboardPage: React.FC = () => {
    ...
    const switchAccount = async () => {
        ...
        try {
            // This prompts MetaMask to show the account picker
            await window.ethereum.request({
                method: 'wallet_requestPermissions',
                params: [{ eth_accounts: {} }],
            });
            // After permission granted, get the new selected account
            const accounts = await window.ethereum.request({ method: 'eth_accounts' });
            if (accounts && accounts.length > 0) {
                showToast(`Switched to ${accounts[0].substring(0, 6)}...${accounts[0].substring(38)}`, 'success');
            }
        } catch (e: any) {
            console.error('Failed to switch account:', e);
            if (e.code !== 4001) { // Don't show error if user just cancelled
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <div style={{
                                    ...
                                    background: isAdmin ? '#F0FFF4' : '#FFF5F5',
                                    color: isAdmin ? '#22543D' : '#C53030',
                                    border: `1px solid ${isAdmin ? '#C6F6D5' : '#FED7D7'}`,
                                }}>
                                    <Shield size={14} />
                                    {isAdmin === null ? 'Checking...' : isAdmin ? '✅ Admin' : '❌ Not Admin'}
                                </div>
                                <button
                            ...
                            {!isAdmin && isAdmin !== null && (
                                <div style={{ fontSize: '0.75rem', color: '#E53E3E', maxWidth: '300px', textAlign: 'right' }}>
                                    Switch to the deployer wallet to manage elections.
                                    Deployer: <a href={`${BLOCK_EXPLORER_URL}/address/0x1d87Bbf912d2A3DC5fF9B2358cE8470DfAE4c7Fa`} target="_blank" style={{ color: '#3182CE' }}>0x1d87...c7Fa</a>
                                </div>
                            )}
                        </div>
                    )}
                ...
                {activeTab === 'elections' && (
                    <div className="page-grid-2">
                        ...
                        <Card style={{ padding: '2rem' }}>
                                ...
                                return (
                                    <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', borderBottom: `1px solid ${theme.colors.gray200}` }}>
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div style={{ fontSize: '0.75rem', color: theme.colors.textSecondary, marginTop: '2px', fontFamily: 'monospace' }}>
                                                ID: {e.id}
                                            </div>
                                            <div style={{ display: 'flex', gap: '1rem', marginTop: '4px', fontSize: '0.8rem', color: theme.colors.textSecondary }}>
                            ...
                            {elections.length === 0 && <p>No elections found on-chain. Create one above!</p>}
                        </Card>
                    </div>
                )}
